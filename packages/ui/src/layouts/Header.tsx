...
export default function Header({ type = "full" }: HeaderProps) {
  const [openMenu, setOpenMenu] = useState<"msg" | "notif" | "account" | null>(
    null,
  );
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 w-full bg-white text-black border-b border-gray-200 ${
        type === "mini" ? "h-12" : "h-16"
      }`}
    >
